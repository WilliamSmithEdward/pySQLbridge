...
QUERIES = [
    ...
    ("cast-to-max-keeps-it-all",
     "SELECT CAST(REPLICATE('a', 50) AS nvarchar(max)) AS s"),
    ("convert-to-max-keeps-it-all",
     "SELECT CONVERT(varchar(MAX), REPLICATE('a', 50)) AS s"),
    ("cast-to-max-of-a-column",
     "SELECT CAST(name AS nvarchar(max)) + '|' AS s FROM people ORDER BY id"),
    ("cast-to-text-keeps-it-all",
     "SELECT DATALENGTH(CAST(REPLICATE('a', 50) AS text)) AS n"),
    ("cast-to-sysname-is-128",
     "SELECT LEN(CAST(REPLICATE('a', 200) AS sysname)) AS n"),
    ("cast-to-char-max-will-not-compile",
     "SELECT CAST('abc' AS char(max)) AS s"),
    ("an-int-too-long-for-varchar", "SELECT CAST(12345 AS varchar(3)) AS s"),
    ("an-int-too-long-for-char", "SELECT CAST(12345 AS char(3)) + '|' AS s"),
    ("a-column-too-long-for-varchar",
     "SELECT CAST(id * 1000 AS varchar(3)) AS s FROM people ORDER BY id"),
    ("an-int-too-long-for-nvarchar", "SELECT CAST(12345 AS nvarchar(3)) AS s"),
    ("a-decimal-too-long-for-varchar", "SELECT CAST(1.25 AS char(3)) AS s"),
    ("a-float-too-long-for-varchar",
     "SELECT CAST(score AS varchar(3)) AS s FROM people WHERE id = 4"),
    ("a-float-too-long-for-nvarchar",
     "SELECT CAST(score AS nvarchar(3)) AS s FROM people WHERE id = 4"),
    ("a-big-number-too-long-for-varchar",
     "SELECT CAST(2147483648 AS varchar(5)) AS s"),
    ("cast-to-a-decimal-rounds", "SELECT CAST(1.239 AS decimal(5,2)) AS n"),
    ("cast-to-a-decimal-away-from-nought",
     "SELECT CAST(-1.25 AS numeric(5,1)) AS n"),
    ("cast-to-a-decimal-of-no-size", "SELECT CAST(1.5 AS decimal) AS n"),
    ("cast-to-dec", "SELECT CAST(1.239 AS dec(5,2)) AS n"),
    ("cast-a-float-to-a-decimal", "SELECT CAST(2.675e0 AS decimal(5,2)) AS n"),
    ("cast-a-decimal-to-a-decimal", "SELECT CAST(2.675 AS decimal(5,2)) AS n"),
    ("cast-a-column-to-a-decimal",
     "SELECT CAST(score AS decimal(5,1)) AS n FROM people ORDER BY id"),
    ("cast-to-a-decimal-too-small", "SELECT CAST(123 AS decimal(3,1)) AS n"),
    ("cast-to-money", "SELECT CAST(1.23456 AS money) AS n"),
    ("cast-to-smallmoney", "SELECT CAST(12.34567 AS smallmoney) AS n"),
    ("cast-to-money-too-small", "SELECT CAST(1e20 AS money) AS n"),
    ("cast-to-a-date-drops-the-time",
     "SELECT CAST('2024-01-02 10:11' AS date) AS d"),
    ("cast-a-column-to-a-date",
     "SELECT CAST([when] AS date) AS d FROM moments ORDER BY mid"),
    ("a-float-in-text", "SELECT CAST(1.0e0 / 3 AS varchar(30)) AS s"),
    ("a-float-in-text-concatenated", "SELECT CONCAT(1.0e0 / 7, '') AS s"),
    ("a-float-too-big-for-six-digits",
     "SELECT CAST(1234567.0e0 AS varchar(30)) AS s"),
    ("a-float-in-scientific", "SELECT CAST(1e20 AS varchar(30)) AS s"),
    ("a-small-float-in-text", "SELECT CAST(0.00001e0 AS varchar(30)) AS s"),
    ("a-float-column-in-text",
     "SELECT CAST(score AS varchar(30)) AS s FROM people ORDER BY id"),
    ("a-float-column-concatenated",
     "SELECT CONCAT(score, '|') AS s FROM people ORDER BY id"),
    ("a-decimal-in-text", "SELECT CAST(1234567.89 AS varchar(20)) AS s"),
    ("a-decimal-concatenated", "SELECT CONCAT(1234567.89, '') AS s"),
    ("a-long-decimal-concatenated", "SELECT CONCAT(123456789012.345, '') AS s"),
    ("a-decimals-trailing-nought", "SELECT CONCAT(1.50, '|') AS s"),
    ("a-decimals-trailing-nought-cast",
     "SELECT CAST(1.50 AS varchar(20)) AS s"),
    ("a-decimal-measured", "SELECT LEN(1234567.89) AS n"),
    ("a-decimal-replaced", "SELECT REPLACE(1234567.89, '7', 'x') AS s"),
    ("a-decimal-in-concat-ws", "SELECT CONCAT_WS('-', 1234567.89, 1) AS s"),
    ("a-negative-decimal-concatenated", "SELECT CONCAT(-1.50, '') AS s"),
    ("a-decimal-cast-to-places-in-text",
     "SELECT CAST(CAST(1.5 AS decimal(5,3)) AS varchar(20)) AS s"),
    ("a-declared-decimal-concatenated",
     "DECLARE @d decimal(12,3) = 1234567.891 SELECT CONCAT(@d, '') AS s"),
    ("money-in-text", "SELECT CAST(CAST(12.5 AS money) AS varchar(30)) AS s"),
    ("money-rounds-to-two-places-in-text",
     "SELECT CAST(CAST(12.567 AS money) AS varchar(30)) AS s"),
    ("a-whole-money-in-text",
     "SELECT CAST(CAST(12 AS money) AS varchar(30)) AS s"),
    ("negative-money-in-text",
     "SELECT CAST(CAST(-12.5 AS money) AS varchar(30)) AS s"),
    ("smallmoney-in-text",
     "SELECT CAST(CAST(12.5 AS smallmoney) AS varchar(30)) AS s"),
    ("money-concatenated", "SELECT CONCAT(CAST(12.5 AS money), '') AS s"),
    ...
    ("cut-after-offset", "SELECT id FROM people ORDER BY id OFFSET"),
    ("cut-after-fetch-next-count",
     "SELECT id FROM people ORDER BY id OFFSET 2 ROWS FETCH NEXT 2"),
    ("cut-after-full", "SELECT COUNT(*) AS n FROM people p FULL"),
    ("cut-after-an-if-condition", "IF 1 = 1"),
    ("ties-with-no-order-by", "SELECT TOP 2 WITH TIES id FROM people"),
    ("ties-with-no-order-by-and-no-from", "SELECT TOP 2 WITH TIES id, name"),
    ("an-aggregate-of-a-column-with-no-table", "SELECT MAX(score)"),
    ("two-aggregates-of-a-column-with-no-table",
     "SELECT MAX(score) - MIN(score)"),
    ("sp-executesql-with-nothing-to-run", "EXEC sp_executesql"),
    ...
    ("non-boolean-in-an-iif", "SELECT IIF(1, 'a', 'b') AS v"),
    ("non-boolean-text-in-an-iif", "SELECT IIF('x', 1, 2) AS v"),
    ("non-boolean-iif-of-a-column", "SELECT IIF(rank, 'a', 'b') AS v FROM people"),
    ("non-boolean-iif-beside-a-condition",
     "SELECT IIF(1 AND 1 = 1, 'a', 'b') AS v"),
    ("non-boolean-iif-in-brackets", "SELECT IIF((1), 'a', 'b') AS v"),
    ("non-boolean-iif-of-a-bit", "SELECT IIF(CAST(1 AS bit), 'a', 'b') AS v"),
    ("a-condition-inside-an-iif", "SELECT IIF(1 = 1, 'a', 'b') AS v"),

    # --- an expression whose every part is the word NULL ---------------------
    ("case-of-only-null", "SELECT CASE WHEN 1 = 1 THEN NULL ELSE NULL END AS v"),
    ("case-of-only-null-no-else", "SELECT CASE WHEN 1 = 1 THEN NULL END AS v"),
    ("case-of-only-null-two-whens",
     "SELECT CASE WHEN 1 = 1 THEN NULL WHEN 2 = 2 THEN NULL ELSE NULL END AS v"),
    ("case-of-only-null-compared", "SELECT CASE 1 WHEN 1 THEN NULL END AS v"),
    ("case-of-one-null-and-one-not",
     "SELECT CASE WHEN 1 = 1 THEN NULL WHEN 2 = 2 THEN 1 END AS v"),
    ("case-of-a-null-with-a-type",
     "SELECT CASE WHEN 1 = 1 THEN CAST(NULL AS int) END AS v"),
    ("case-of-a-null-and-a-value",
     "SELECT CASE WHEN 1 = 1 THEN NULL ELSE 'x' END AS v"),
    ("case-of-only-null-over-rows",
     "SELECT CASE WHEN rank = 1 THEN NULL ELSE NULL END AS v FROM people"),
    ("iif-of-only-null", "SELECT IIF(1 = 1, NULL, NULL) AS v"),
    ("iif-of-one-null", "SELECT IIF(1 = 1, NULL, 1) AS v"),
    ("coalesce-of-only-null", "SELECT COALESCE(NULL, NULL) AS v"),
    ("coalesce-of-three-nulls", "SELECT COALESCE(NULL, NULL, NULL) AS v"),
    ("coalesce-with-one-value", "SELECT COALESCE(NULL, NULL, 1) AS v"),
    ("nullif-of-a-null-first", "SELECT NULLIF(NULL, 1) AS v"),
    ("nullif-of-two-nulls", "SELECT NULLIF(NULL, NULL) AS v"),
    ("nullif-of-a-null-second", "SELECT NULLIF(1, NULL) AS v"),
    ("isnull-of-two-nulls", "SELECT ISNULL(NULL, NULL) AS v"),
    ("greatest-of-two-nulls", "SELECT GREATEST(NULL, NULL) AS v"),
    ...
    ("qualifier-in-a-where", "SELECT id FROM people WHERE m.id = 1"),
    ("qualifier-in-an-order-by", "SELECT id FROM people ORDER BY m.id"),
    ("qualifier-in-a-group-by",
     "SELECT COUNT(*) AS n FROM people GROUP BY m.team"),
    ("qualifier-in-a-select-list-over-a-join",
     "SELECT x.id FROM people p JOIN tasks t ON t.owner = p.id"),
    ("qualifier-in-a-join-condition",
     "SELECT p.id FROM people p JOIN tasks t ON x.owner = p.id"),
    ("qualifier-of-an-apply",
     "SELECT y.v FROM people p CROSS APPLY (VALUES (1)) AS x(v)"),
    ("qualifier-inside-an-aggregate",
     "SELECT MAX(m.id) AS n FROM people"),
    ("star-prefix-of-no-table", "SELECT x.* FROM people"),
    ("qualifier-with-a-schema",
     "SELECT dbo.people.id AS v FROM people ORDER BY v"),
    ("qualifier-of-a-cte",
     "WITH c AS (SELECT id FROM people) SELECT c.id AS v FROM c ORDER BY v"),
    ("qualifier-of-a-table-a-cte-replaced",
     "WITH c AS (SELECT id FROM people) SELECT people.id AS v FROM c"),

    # --- an aggregate with no table, which reduces the one row there is ----
    ("no-from-count", "SELECT COUNT(*) AS n"),
    ("no-from-count-times-two", "SELECT COUNT(*) * 2 AS n"),
    ("no-from-count-big", "SELECT COUNT_BIG(*) AS n"),
    ("no-from-count-where-false", "SELECT COUNT(*) AS n WHERE 1 = 0"),
    ("no-from-max", "SELECT MAX(5) AS n"),
    ("no-from-max-where-false", "SELECT MAX(5) AS n WHERE 1 = 0"),
    ("no-from-sum-plus-one", "SELECT SUM(2) + 1 AS n"),
    ("no-from-avg", "SELECT AVG(4) AS n"),
    ("no-from-min-of-text", "SELECT MIN('b') AS n"),
    ("no-from-count-distinct", "SELECT COUNT(DISTINCT 1) AS n"),
    ("no-from-string-agg", "SELECT STRING_AGG('a', ',') AS n"),
    ("no-from-a-value-and-a-count", "SELECT 1 AS a, COUNT(*) AS n"),
    ("no-from-two-aggregates", "SELECT COUNT(*) AS n, MAX(3) AS m"),
    ("no-from-having-true", "SELECT COUNT(*) AS n HAVING COUNT(*) = 1"),
    ("no-from-having-false", "SELECT COUNT(*) AS n HAVING COUNT(*) > 5"),
    ("no-from-having-without-an-aggregate", "SELECT 1 AS v HAVING 1 = 1"),
    ("no-from-count-of-a-variable",
     "DECLARE @p int SELECT COUNT(@p) AS n"),
    ("no-from-sum-of-a-variable",
     "DECLARE @p int = 7 SELECT SUM(@p) AS n"),
    ("no-from-distinct-count", "SELECT DISTINCT COUNT(*) AS n"),
    ("no-from-top-count", "SELECT TOP 1 COUNT(*) AS n"),
    ("no-from-count-ordered", "SELECT COUNT(*) AS n ORDER BY 1"),
    ("no-from-group-by", "SELECT COUNT(*) AS n GROUP BY 1"),
    ("no-from-group-by-a-value", "SELECT 1 AS v GROUP BY 1"),
